...
def build_comparison_trajectory(
    reports: list[dict],
) -> Optional[go.Figure]:
    """
    Overlay multiple researchers' trajectories on one chart.
    reports: list of parsed audit JSON dicts.
    """
    if not reports:
        return None

    num_reports = len(reports)
    #colors = ["#EAB308", "#22C55E", "#3B82F6", "#EF4444", "#8B5CF6", "#EC4899"]
